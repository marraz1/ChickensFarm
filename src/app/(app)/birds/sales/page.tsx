...
export default async function BirdSalesPage() {
  const { farm } = await requireActiveFarm();
  const transactions = await listBirdTransactions(farm.id, "SALE");
  ...
  return (
    <div>
      ...
      />
      <BirdTransactionList transactions={transactions} type="SALE" />
    </div>
  );
}
