...
export default async function EditBirdSalePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // A purchase id under the sales route is as much a wrong address as a missing
  // record — the form would silently rewrite its direction on save.
  if (!transaction || transaction.type !== "SALE") notFound();
  const page = birdTransactionPages.SALE;
